...
static int alloc_block(void) // 分配一个数据块,返回数据块号
{
    ...
    unsigned char con=128; // 1000 0000b 128=2**7
    int flag=0;
    ...
    while(bitbuf[cur]&con) //在一个字节中找具体的某一个bit，通过按位与运算判定是否找到
    {
        con=con/2;//查找8次
        flag++;
    }
...
static unsigned short reserch_file(char tmp[9],int file_type,unsigned short *inode_num,unsigned short *block_num,unsigned short *dir_num)
{
    ...
    j=0;
    while(j<inode_area[0].i_blocks)//数据块数
    {
        reload_dir(inode_area[0].i_block[j]);
        k=0;
        while(k<32)//目录项缓冲区dir[32],一次只能读入32个
        {
            if(!dir[k].inode||dir[k].file_type!=file_type||strcmp(dir[k].name,tmp))//inode号，文件类型，名字都必须满足
            {
            	k++;
            }
            else//如果找到
...
void cat(char tmp[9],int type)
{
    unsigned short tmpno,i,j,k,flag;
    reload_inode_entry(current_dir);
    if(!reserch_file(tmp,type,&i,&j,&k))//是否存在该文件
    {
        if(inode_area[0].i_size==4096)//空间已满
        {
            printf("Directory has no room to be alloced!\n");
            return;
        }
...
void rmdir(char tmp[9])
{
    unsigned short i,j,k,flag;
    unsigned short m,n;
    if(!strcmp(tmp,"..")||!strcmp(tmp,"."))//这两个目录无法删除
    {
...
void write_file(char tmp[9]) // 写文件
{
    ...
    if(flag)
    {
        ...
        {
            reload_inode_entry(dir[k].inode);
            if(!(inode_area[0].i_mode&2)) // i_mode:111b:读,写,执行;2==010
            {
...
void ls(void)
{
    ...
    unsigned short i,j,k,flag;
    i=0;
    reload_inode_entry(current_dir);//在当前的索引下
    while(i<inode_area[0].i_blocks)
    {
        k=0;
        reload_dir(inode_area[0].i_block[i]);
        while(k<32)//缓冲区最多支持一次读入32个
        {
            if(dir[k].inode)//若存在索引
            {
                printf("%s",dir[k].name);
                if(dir[k].file_type==2)//类型为目录
                {
                    j=0;
                    reload_inode_entry(dir[k].inode);
                    if(!strcmp(dir[k].name,".."))//上层目录
                    {
                    	while(j++<13)
                    	{
                    		printf(" ");
                    ...
                    else if(!strcmp(dir[k].name,"."))//当前目录
                    {
                    	while(j++<14)
                    	{
                    		printf(" ");
                    ...
                    printf("<DIR>          ");
                    switch(inode_area[0].i_mode&7)//7==111 
                    {
                ...
                else if(dir[k].file_type==1)//类型为文件
                {
                    j=0;
                    reload_inode_entry(dir[k].inode);
